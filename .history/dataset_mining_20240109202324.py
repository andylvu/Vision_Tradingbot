...
class DataMining:
    ...
    # function to save image and first two labels to table
    def insert_label1(self, image, trend, phase):

        insert_query = "INSERT INTO trading_data (image_data, trend, phase) VALUES (?, ?, ?)"
        self.db_cursor.execute(insert_query, (image, trend, phase))
        self.db_connection.commit()                       


    # function to save third label to table
    def insert_label2(self, after):
        insert_query = "INSERT INTO trading_data (after) VALUES (?)"
        self.db_cursor.execute(insert_query, (after))
        self.db_connection.commit()